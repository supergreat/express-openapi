...
/**
 * Returns this layer's handle, in case it's wrapped by datadog.
 */
function getLayerHandle(layer) {
  if (!layer.handle) {
    return null
  } else if (layer.handle._datadog_orig) {
   return layer.handle._datadog_orig
  } else {
   return layer.handle
  }
}
...
module.exports = function generateDocument (baseDocument, router) {
  ...
  router && router.stack.forEach((_layer) => {
    iterateStack('', null, _layer, (path, routeLayer, layer) => {
      const handle = getLayerHandle(layer)

      if (!handle) {
        return
      }

      const schema = getSchema(handle)
